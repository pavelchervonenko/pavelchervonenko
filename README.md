# Pavel Chervonenko

**Backend Developer** · Java · Spring Boot

I build backend systems in Java — REST APIs, web applications, CLI tools and cryptographic libraries.
Currently studying at **Moscow Aviation Institute** (3rd year) and have completed the **Hexlet Java Developer** professional program.

[![Telegram](https://img.shields.io/badge/@pavelchervonenko-2CA5E0?style=flat-square&logo=telegram&logoColor=white)](https://t.me/pavelchervonenko)

---

## Tech Stack

**Core**

![Java](https://img.shields.io/badge/Java_21-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Gradle](https://img.shields.io/badge/Gradle-02303A?style=flat-square&logo=gradle&logoColor=white)

**Spring Ecosystem**

![Spring Boot](https://img.shields.io/badge/Spring_Boot_3-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=flat-square&logo=springsecurity&logoColor=white)
![Spring Data JPA](https://img.shields.io/badge/Spring_Data_JPA-6DB33F?style=flat-square&logo=spring&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white)

**Data & ORM**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat-square&logo=hibernate&logoColor=white)
![HikariCP](https://img.shields.io/badge/HikariCP-2496ED?style=flat-square)
![MapStruct](https://img.shields.io/badge/MapStruct-EF2D5E?style=flat-square)
![Lombok](https://img.shields.io/badge/Lombok-BC2330?style=flat-square)

**Web & API**

![REST API](https://img.shields.io/badge/REST_API-FF6C37?style=flat-square)
![Swagger](https://img.shields.io/badge/Swagger_/_OpenAPI-85EA2D?style=flat-square&logo=swagger&logoColor=black)
![Javalin](https://img.shields.io/badge/Javalin-F37626?style=flat-square)

**Testing & Quality**

![JUnit 5](https://img.shields.io/badge/JUnit_5-25A162?style=flat-square&logo=junit5&logoColor=white)
![MockWebServer](https://img.shields.io/badge/MockWebServer-25A162?style=flat-square)
![SonarCloud](https://img.shields.io/badge/SonarCloud-F3702A?style=flat-square&logo=sonarcloud&logoColor=white)
![Sentry](https://img.shields.io/badge/Sentry-362D59?style=flat-square&logo=sentry&logoColor=white)

**DevOps & Infrastructure**

![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)

**Also worked with**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)

---

## Projects

| Project | Description | Stack |
|---|---|---|
| [Task Manager](https://github.com/pavelchervonenko/java-project-99) | Full-featured task management web app with JWT auth, roles and labels | Spring Boot 3, JWT, JPA, PostgreSQL, Docker |
| [Page Analyzer](https://github.com/pavelchervonenko/java-project-72) | Web service for SEO analysis: status code, title, h1, meta description | Javalin, PostgreSQL, HikariCP, MockWebServer |
| [Data Validator](https://github.com/pavelchervonenko/java-project-78) | Fluent validation library for strings, numbers and maps (inspired by Yup) | Java, JUnit 5 |
| [Diff Calculator](https://github.com/pavelchervonenko/java-project-71) | CLI tool to diff JSON/YAML files with stylish, plain and JSON output | Java, Jackson, SnakeYAML |
| [Cryptography](https://github.com/pavelchervonenko/cryptography-5sem) | DES, AES (Rijndael), RSA, GF(2⁸) — implemented from scratch | Java 17, multithreading |
| [DeBugg](https://github.com/pavelchervonenko/DeBugg) | Team project: RAG-like ML module with GigaChat API integration | Python, LaBSE, GigaChat API |
| [Machine Learning](https://github.com/pavelchervonenko/machine-learning-5sem) | Gradient boosting, optimization, model tuning | Python, scikit-learn, Optuna |
| [System Programming](https://github.com/pavelchervonenko/system-programming) | Shell, IPC, sockets, threading, dining philosophers | C, C++ |

---

## Currently Learning

- 📖 Reading *Designing Data-Intensive Applications* — Martin Kleppmann
- 📡 Exploring **Apache Kafka** — event streaming and message brokers
- 🏗️ Deepening knowledge of **microservices architecture**

---

## GitHub Stats

![Pavel's GitHub stats](https://github-readme-stats.vercel.app/api?username=pavelchervonenko&show_icons=true&hide_border=true&theme=default&hide=contribs)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=pavelchervonenko&layout=compact&hide_border=true&theme=default)

---


